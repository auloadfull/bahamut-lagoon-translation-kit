...
namespace script {
  //The encoded chapter/field text blob outgrew the $f2-$f6 text banks once
  //every chapter carried 3-byte Korean glyphs. It now lives in the otherwise
  //unused ExHiROM second-chip banks $70-$7d (file 0x700000+); the matching
  //base bank is applied in redirection.enable. $7e+ is WRAM, so cap below it.
  seek($700000)
...
namespace opening {
  inline insert(define id) {
    insert block{id}, "../en/binaries/lists/opening-{id}-bpp2.bin"
  }
  ...
  seek(cursors[1])
  insert(9);insert(a);insert(b);insert(c);insert(d);insert(e);insert(f);insert(g);insert(h);
  cursors[1] = pc()
  assert(pc() <= $418000)
}
...
namespace ending {
  inline insert(define id) {
    insert block{id}, "../en/binaries/lists/ending-{id}-bpp2.bin"
  }
  ...
  seek(cursors[3])
  insert(7);insert(8);insert(9);insert(a);insert(b);insert(c);insert(d)
  cursors[3] = pc()
  assert(pc() <= $438000)
}
...
//koLargeFont is split into fixed 256-glyph sub-blobs (24,576 bytes each) so
//every file fits a single 32KB asset bank. Compact index bits 8-9 select the
//sub-blob at runtime; unused sub-blobs are zero-length files.
namespace koLargeFont {
  namespace sub0 {
    insert("fonts/font-ko-large-0", normal)
    insert("fonts/font-ko-large-0", yellow)
  }
  namespace sub1 {
    insert("fonts/font-ko-large-1", normal)
    insert("fonts/font-ko-large-1", yellow)
  }
  namespace sub2 {
    insert("fonts/font-ko-large-2", normal)
    insert("fonts/font-ko-large-2", yellow)
  }
  namespace sub3 {
    insert("fonts/font-ko-large-3", normal)
    insert("fonts/font-ko-large-3", yellow)
  }
...
namespace endingFont {
  insert("fonts/font-ending", data)
}
...
namespace dragons {
  ...
  //per-type Korean particle flags (0=가/로, 1=이/으로, 2=이/로) generated by
  //apply_chapter_ko_poc.py from the synced KO type names
  insert("lists/dragons", particles)
}
...
namespace items {
  ...
  //per-item Korean particle flags (0=를, 1/2=을) generated by
  //apply_chapter_ko_poc.py from the KO item-name list
  insert("lists/items", particles)
}
